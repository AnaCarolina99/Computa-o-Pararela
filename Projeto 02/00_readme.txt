...
Para este trabalho foi aproveitado parte do Projeto 01 da disciplina de computação paralela. Importamos os mesmos resultados e mesmas métricas utilizadas no trabalho anterior. Assim como lógica de aplicação da paralelização. Desta vez utilizamos a paralelização usando a linguagem de programação CUDA da NVIDIA. Desta vez somente a função void RNA_CalcularSaida() foi paralelizada. Os resultados das Redes Neurais em um sigle core, multi core e CUDA seguem demostrados e explicados a baixo.
...
--redeNeural_multicore.c 	// Código da Rede Neural em paralelo em Multicore.
--main.cu 		 	// Arquivo que contém código fonte em CUDA para chamada e simulação da Rede Neural.
--redeNeural.cu 	 	// Código da Rede Neural em paralelo em CUDA.
--compile_execution_sample.JPG	// Print Screen de exemplos de compilação e execução no servidor PARCODE.
...
Utilizando CUDA:
 -2.905s
 Verificamos um SpeedUp bastante significativo utlizando o CUDA. No caso para uma Rede Neural que tem vários neurônios que calcularam uma saída várias e várias vezes. A quantidades elevada de Cores no nosso caso CUDA Cores ajuda bastante no cálculo final. O SpeedUp atingido foi de 3,65.
